...
for run in range(num_runs):
    ...
    epochs = 1
    for epoch in range(epochs):
        for batch_idx, (data, target) in enumerate(nonthree_train_loader):
            data = data.view(-1, 28*28)
            optimizer.zero_grad()
            ...
            if batch_idx % 100 == 0:
                print(
                    f"Train Epoch: {epoch} [{batch_idx * len(data)}/{num_nonthree_train_samples} ({100. * batch_idx / len(nonthree_train_loader):.0f}%)]\tLoss: {loss.item():.6f}")
    ...
    run_results['Retrained']['Forget Set Accuracy'] = test(
        retrained_model, three_train_loader, dname="Training data with only 3s", printable=False)
    run_results['Retrained']['Retain Set Accuracy'] = test(
        retrained_model, nonthree_train_loader, dname="Training data except 3s", printable=False)
    run_results['Retrained']['Test (Forget) Set Accuracy'] = test(
        retrained_model, three_test_loader, dname="Test data with only 3s", printable=False)
    run_results['Retrained']['Test (Retain) Set Accuracy'] = test(
        retrained_model, nonthree_test_loader, dname="Test data except 3s", printable=False)

    print("Retraining Complete")

    results.append(run_results)

with open("/home/aniket7/unlearning_exps/selective_reinitialization_unlearning_mnist_3_fraction_retraining/selective_reinitialization_unlearning_mnist_3_fraction_retraining.json", "w") as f:
