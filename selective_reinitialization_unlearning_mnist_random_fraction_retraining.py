# -*- coding: utf-8 -*-
"""selective_reinitialization_unlearning_mnist_random_fraction_retraining.ipynb
...
total_indices = list(range(len(traindata)))
np.random.shuffle(total_indices)

# Calculate the split index
split_idx = int(np.floor(percent_forget * len(traindata)))
# split_idx = 1

# Split indices into two parts
indices_forget = total_indices[:split_idx]
indices_retain = total_indices[split_idx:]

forget_data_loader = torch.utils.data.DataLoader(traindata, batch_size=64,
                                                 sampler=torch.utils.data.SubsetRandomSampler(indices_forget))
retain_data_loader = torch.utils.data.DataLoader(traindata, batch_size=64,
                                                 sampler=torch.utils.data.SubsetRandomSampler(indices_retain))
num_forget_samples = len(indices_forget)
num_retain_samples = len(indices_retain)
...
for run in range(num_runs):
    ...
    # test(model, forget_data_loader, dname="Forget data")
    # test(model, retain_data_loader, dname="Retain data")
    ...
    run_results['After Training']['Forget Set Accuracy'] = test(
        model, forget_data_loader, dname="Training data with only 3s", printable=False)
    run_results['After Training']['Retain Set Accuracy'] = test(
        model, retain_data_loader, dname="Training data except 3s", printable=False)
    ...
    utilities = get_node_utilities(
        model, retain_data_loader, forget_data_loader)

    utilities

    """# Selective Reinitialization"""

    reinitialize_nodes(model, utilities, alpha=0.1)

    # test(model, all_data_train_loader, dname="Training data")
    # test(model, all_data_test_loader, dname="Test data")
    # test(model, forget_data_loader, dname="Training data with only 3s")
    # test(model, retain_data_loader, dname="Training data except 3s")
    ...
    np.random.shuffle(indices_retain)
    small_retain_index = indices_retain[:int(0.01*num_retain_samples)]
    retain_small_data_loader = torch.utils.data.DataLoader(traindata, batch_size=64,
                                                           sampler=torch.utils.data.SubsetRandomSampler(small_retain_index))
    num_retain_small_samples = len(small_retain_index)
    ...
    # test(ft_model, forget_data_loader, dname="Training data with only 3s")
    # test(ft_model, retain_data_loader, dname="Training data except 3s")
    ...
    run_results['After Fine Tuning']['Forget Set Accuracy'] = test(
        ft_model, forget_data_loader, dname="Training data with only 3s", printable=False)
    run_results['After Fine Tuning']['Retain Set Accuracy'] = test(
        ft_model, retain_data_loader, dname="Training data except 3s", printable=False)

    retrained_model = SimpleFNN()
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.SGD(retrained_model.parameters(), lr=0.01, momentum=0.9)
    ...
    for epoch in range(epochs):
        for batch_idx, (data, target) in enumerate(retain_data_loader):
            ...
            if batch_idx % 100 == 0:
                print(
                    f"Train Epoch: {epoch} [{batch_idx * len(data)}/{num_retain_samples} ({100. * batch_idx / len(retain_data_loader):.0f}%)]\tLoss: {loss.item():.6f}")

    print("Fine Tuning Complete")

    run_results['Retrained'] = {}
    run_results['Retrained']['Test Set Accuracy'] = test(
        retrained_model, all_data_test_loader, dname="Test data", printable=False)
    run_results['Retrained']['Forget Set Accuracy'] = test(
        retrained_model, forget_data_loader, dname="Training data with only 3s", printable=False)
    run_results['Retrained']['Retain Set Accuracy'] = test(
        retrained_model, retain_data_loader, dname="Training data except 3s", printable=False)

    print("Retraining Complete")

    results.append(run_results)

with open("/home/aniket7/unlearning_exps/selective_reinitialization_unlearning_mnist_random_fraction_retraining/selective_reinitialization_unlearning_mnist_random_fraction_retraining.json", "w") as f:
